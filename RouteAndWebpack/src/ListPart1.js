...
class ListPart1 extends React.Component {
  ...
  render() {
    ...
    _titles = _titles.map(function(item, index) {
      i++;
      return <ListPart2 item={item} key={index} i_id={i} />;
    });
    return (
      <div>
        {this.props.test}
        <h1> List </h1> <ul> {_titles} </ul>{" "}
      </div>
    );
  }
}
...
class ListPart2 extends React.Component {
  ...
  listClickker(e) {
    ...
    let collectedId = postData.find(q => q.id.toString() === tarId.toString());
    ...
    //window.location.href = "/Second" + "?var1=33&&var2=11";
    ...
    ReactDOM.render(
      <PostDetail
        pD={postData}
        uD={userData}
        takenID={collectedId1}
        takenUsername={dUserName}
        takenPhone={dPhone}
        takenName={dName}
        takenTitle={dTitle}
        takenBody={dBodyCount}
        takenEmail={dEmail}
      />,
  ...
  render() {
    let a = postData.find(q => q.title === this.props.item);

    return (
      <div onClick={this.listClickker} className="Listdiv" id={this.props.i_id}>
        <span> Post Title: {this.props.item} </span> <p />
        <span>
          {" "}
          Username: <GetUsername no={a.userId} vTitle={this.props.item} />
        </span>
      </div>
    );
  }
...
function GetUsername(props) {
  ...
  let t = postData.find(q => q.title === props.vTitle);
  let vv = t.userId;
  if (props.no.toString() === vv.toString()) {
    let c = userData.find(q => q.id === vv);
